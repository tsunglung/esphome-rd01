...
class RD01 : public PollingComponent, public UARTDevice
{
public:
  RD01(UARTComponent *parent) : UARTDevice(parent) {}
  ...
  void handlePeriodicData(char *buffer, int len)
  {
    ...
    */
    float newMovingTargetDistance = twoByteToInt(buffer[9], buffer[10]) / 10;
    ...
    float newStillTargetDistance = twoByteToInt(buffer[12], buffer[13]) / 10;
    ...
    float newDetectDistance = twoByteToInt(buffer[15], buffer[16]) / 10;
  ...
  void setDistancesAndStillSensitive(int DistanceRange1, int DistanceRange2, int StillSensitive)
  {
    char cmd[2] = {0x64, 0x00};
    char value[18] = {0x00, 0x00, lowByte(DistanceRange1), highByte(DistanceRange1), 0x00, 0x00, 0x01, 0x00, lowByte(DistanceRange2), highByte(DistanceRange2), 0x00, 0x00, 0x02, 0x00, lowByte(StillSensitive), highByte(StillSensitive), 0x00, 0x00};
    sendCommand(cmd, value, 18);
    queryParameters();
  }
